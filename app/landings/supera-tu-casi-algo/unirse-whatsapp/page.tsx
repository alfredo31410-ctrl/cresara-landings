...
export const metadata: Metadata = {
  title: "Abriendo WhatsApp | Cressara",
...
export default async function JoinPage() {
  ...
  if (!(await readRegistrationSession(config.slug))) {
    redirect(`/landings/${config.slug}`);
  }
  if (!(await readWhatsAppIntent(config.slug))) {
    redirect(`/landings/${config.slug}/gracias`);
  }
    ...
    <main className={styles.page}>
      <section className={styles.card}>
        ...
        <WhatsAppRedirectClient
          ...
          contentName={config.tracking!.joinGroupContentName}
          groupUrl={getWhatsAppGroupUrl(config)}
          classNames={{
